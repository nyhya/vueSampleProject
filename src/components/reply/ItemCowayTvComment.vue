...
<script>
import { deleteCowayComment, putCowayComment } from '@/api/cowayVideo.js';
export default {
	props: {
		comment: {
			type: Object,
	...
	methods: {
		...
		onClickDelete() {
			const vm = this;
			const userData = {
				cntsCd: this.comment.cntsCd,
				cmntCd: this.comment.cmntCd,
			};
			deleteCowayComment(userData)
				.then(function (response) {
					vm.$emit('replyCowayApi');
				})
		...
		onClickInputComment() {
			this.showToggle = !this.showToggle;
			/**
			 *  "commentCode" : "" //수정할때 넣는거
     			"contentsCode" : "17839FCDB96BBGVPILYJ", 필수
